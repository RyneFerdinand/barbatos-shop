...
@section('content')
    <div class="w-1/2 mx-auto my-20">
        @foreach ($transactions as $transaction)
            ...
            <div id="trigger{{$transaction->id}}" class="bg-white grid-cols-[50%_25%_25%] rounded-md p-2 hidden ease-in transition-all origin-top duration-1000">
                ...
                @foreach ($transaction->details as $detail)
                    <p class="border border-gray-600 border-l-0 pl-1">{{$detail->product->name}}</p>
                    <p class="border border-gray-600 pl-1">{{$detail->qty}}</p>
                    <p class="border border-gray-600 border-r-0 pl-1">IDR {{$detail->product->price * $detail->qty}}</p>
                    <?php
                        $totalQty += $detail->qty;
                ...
                <h4 class="font-semibold text-lg border border-gray-600 border-l-0 border-b-0 pl-1">Total</h4>
                <p class="text-lg border border-gray-600 border-b-0 pl-1">{{$totalQty}} Item(s)</p>
                <p class="text-lg border border-gray-600 border-r-0 border-b-0 pl-1">IDR {{$totalPrice}}</p>
            </div>
        @endforeach
    </div>
