...
<div class="flex flex-row w-[100%] bg-white justify-between mb-3">
    ...
    <div class="flex flex-row pr-3">
        <a href="{{url('/update/'.$product->id)}}" class="flex items-center justify-center bg-white text-rounded-md border-[2px] border-yellow-400 h-[40px] w-[40px] mt-2 mr-2 rounded-md hover:bg-yellow-400 hover:text-white fa fa-pencil text-yellow-400 transition-all"></a>
        <form enctype="multipart/form-data" method="POST" action="{{url('/delete/'.$product->id)}}">
            {{method_field('DELETE')}}
            {{csrf_field()}}
